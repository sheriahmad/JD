    ...
    protected void IDMissingLinks()
{
  ...
  //*********************************** JPE Smart Buyer
    ...
    protected string buildDeliveryDate(string thisType, string thisKey, decimal decqtyBO, string thisPN, int stcKey)
    {
        ...
        if (stcKey == 2 || stcKey == 6 || stcKey == 12) //------ woo
        {
        ...
        else if (stcKey == 3 || stcKey == 3 || stcKey == 10 || stcKey == 20)
        {
...
strMyAccountType == "jpe" || strMyAccountType == "admin")
    ...
    { 
        ...
        int rowCounter = - 1;
        if (dtCacheMaster.Rows[thisRow]["pmast_P_STC_AUTO_KEY"].ToString() != "2" && dtCacheMaster.Rows[thisRow]["pmast_P_STC_AUTO_KEY"].ToString() != "20" && dtCacheMaster.Rows[thisRow]["pmast_P_STC_AUTO_KEY"].ToString() != "4" && dtCacheMaster.Rows[thisRow]["pmast_P_STC_AUTO_KEY"].ToString() != "6" && dtCacheMaster.Rows[thisRow]["pmast_P_STC_AUTO_KEY"].ToString() != "12")
        {
      ...
      if (dtCacheMaster.Rows.Count > 0)
        {
            foreach (DataRow myControlRow in dtCacheMaster.Rows)
            {
                ...
                if (intCurRow > -1 && showthisRow)
                {
                    ...
                    if (decCost > 0 && (intStockCategoryKey == 2 || intStockCategoryKey == 4 || intStockCategoryKey == 6 || intStockCategoryKey == 12 || intStockCategoryKey == 20 || (intStockCategoryKey == 3 && intCurRow == controlRowCounter)))
                    {
        ...
        for (lp = 0; lp <= swCounter; lp++)
        {
            if (SPBmasterUNIFIEDList[lp, 2] == "SearchPanel" || SPBmasterUNIFIEDList[lp, 2] == "List")
            {
                        ...
                        if (dtCacheMaster.Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow myfinalRow in dtCacheMaster.Rows)
                            {
                                ...
                                if (strThisPartNumberStripped.Length >= intThisSearchLength)
                                {
                                    ...
                                    if (str1 == str2 && decCost > 0 && (str3 == "2" || str3 == "4" || str3 == "6" || str3 == "12" || str3 == "20"))
                                    {
                ...
                if (!blnPartialMatchFound && !blnSingleMatchFound)
                {
                    ...
                    if (strTest.Length > 2)
                    {
                        strWhereString = " WHERE p.PN_STRIPPED Like'" + GenUtils.MakeUCaseNumChar(leadLookup) + "%' AND ( P.STC_AUTO_KEY = 2 OR P.STC_AUTO_KEY = 4 OR P.STC_AUTO_KEY = 6 OR P.STC_AUTO_KEY = 12 OR P.STC_AUTO_KEY = 20) ";
                        //--- OK No Match so load the parts that start with 
